...
3. Analýza rizik.
4. Doporučené kroky.
Piš formálně, odstavcově, v češtině. Nepoužívej markdownové bloky, vrať čistý text. Neuváděj doporučení typu vyhledat dluhového poradce, obrátit se na dluhovou poradnu nebo zahájit další mapování, protože tato zpráva už vzniká v rámci dluhového poradenství.`;
        ...
        if (responseText) {
          const sanitizedAiText = sanitizeAiReportText(responseText);
          appData.report.generated_text = sanitizedAiText;
          appData.report.edited_text = sanitizedAiText;
          touch();
    ...
    function sanitizeAiReportText(text) {
      const cleanedLines = String(text || '')
        .replace(/\r\n/g, '\n')
        .replace(/\r/g, '\n')
        .split('\n')
        .filter(line => !/dluhov[ýy]\s+poradc|vyhledat\s+(dluhov[ée]ho\s+)?poradc|obr[aá]tit\s+se\s+na\s+(dluhov[ée]ho\s+)?poradc/i.test(line))
    ...
    function reportTextToWordHtml(text) {
      ...
      const blocks = [];
      let bulletItems = [];
      let insertedDebtTable = false;
      let titleHandled = false;

      function buildDebtTableHtml() {
        const rows = (appData.obligations || []).map(item => `
          <tr>
            <td style="border: 1px solid #d8cabd; padding: 8px 10px; vertical-align: top;">${escapeHtml(item.veritel_nazev || 'Neuvedeno')}</td>
            <td style="border: 1px solid #d8cabd; padding: 8px 10px; vertical-align: top;">${escapeHtml(fmtCurrency(item.vyse_dluhu || 0))}</td>
            <td style="border: 1px solid #d8cabd; padding: 8px 10px; vertical-align: top;">${escapeHtml(item.stav_dluhu || 'Neuvedeno')}</td>
          </tr>`).join('');

        if (!rows) return '';

        return `
          <table style="width: 100%; border-collapse: collapse; margin: 0 0 16px; font-size: 11pt;">
            <tr>
              <th style="border: 1px solid #cbb9a6; background: #f3e5d6; padding: 8px 10px; text-align: left;">Věřitel</th>
              <th style="border: 1px solid #cbb9a6; background: #f3e5d6; padding: 8px 10px; text-align: left;">Výše dluhu</th>
              <th style="border: 1px solid #cbb9a6; background: #f3e5d6; padding: 8px 10px; text-align: left;">Stav dluhu</th>
            </tr>
            ${rows}
          </table>`;
      }
      ...
      function isDebtSectionHeading(line) {
        return normalizePlainText(line).includes('zavazky');
      }

      function isDebtBulletLine(line) {
        const normalized = normalizePlainText(line);
        return normalized.startsWith('- ') && normalized.includes('| vyse:') && normalized.includes('| stav:');
      }

      function flushBullets() {
        if (!bulletItems.length) return;
        blocks.push(`<ul style="margin: 0 0 14px 22px; padding: 0; text-align: justify;">${bulletItems.join('')}</ul>`);
        bulletItems = [];
      }

      function ensureDebtTableInserted() {
        if (insertedDebtTable) return;
        const tableHtml = buildDebtTableHtml();
        if (!tableHtml) return;
        blocks.push(tableHtml);
        insertedDebtTable = true;
      }
      ...
      lines.forEach((rawLine, index) => {
        ...
        if (/^\d+\.\s+/.test(line)) {
          ...
          if (isDebtSectionHeading(line)) {
            ensureDebtTableInserted();
          }
          return;
        }

        if (/^- /.test(line)) {
          if (isDebtBulletLine(line)) return;
          bulletItems.push(`<li style="margin: 0 0 6px;">${escapeHtml(line.replace(/^- /, ''))}</li>`);
          return;
        }

        blocks.push(`<p style="margin: 0 0 12px; line-height: 1.55; text-align: justify;">${escapeHtml(line)}</p>`);
      });

      flushBullets();

      if (!insertedDebtTable) {
        const tableHtml = buildDebtTableHtml();
        if (tableHtml) {
          const debtSection = `<h2 style="font-family: Calibri, Arial, sans-serif; font-size: 13.5pt; margin: 18px 0 8px; color: #1f2937;">Přehled závazků</h2>${tableHtml}`;
          if (titleHandled && blocks.length > 1) {
            blocks.splice(1, 0, debtSection);
          } else {
            blocks.push(debtSection);
          }
        }
      }
    ...
    function exportReportWord() {
      ...
      const preHtml = "<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'><head><meta charset='utf-8'><title>ZprĂˇva</title><style>body{font-family:Calibri,Arial,sans-serif;color:#111827;font-size:11pt;margin:32px;} p,li{font-size:11pt;text-align:justify;} ul{list-style-type:disc;text-align:justify;} h1,h2{text-align:left;}</style></head><body>";
      const postHtml = "</body></html>";
